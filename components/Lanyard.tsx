...
const LANYARD_TEX_URL = '/assets/lanyard/Lanyardss.png';
...
function Band({ maxSpeed = 50, minSpeed = 0, isMobile = false }: { maxSpeed?: number; minSpeed?: number; isMobile?: boolean }) {
  ...
  useFrame((state, delta) => {
    ...
    if (fixed.current && j3.current && j2.current && j1.current && card.current && band.current) {
      ...
      try {
        ...
        console.warn('Error setting curve points:', error);
      }
  ...
  const lineGeometry = useMemo(() => new (MeshLineGeometry as any)(), []);
  const lineMaterial = useMemo(() => {
    const mat = new (MeshLineMaterial as any)({
      depthTest: false,
      depthWrite: false,
      transparent: true,
    ...
    // MeshLine expects a Vector2 on some builds; keep it stable.
    mat.resolution = new THREE.Vector2(1000, isMobile ? 2000 : 1000);
    return mat;
  }, [isMobile, texture]);
